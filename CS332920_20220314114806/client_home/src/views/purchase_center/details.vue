<template>
	<div class="diy_details page_purchase_center" id="purchase_center_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_purchase_center'>
							<div_purchase_center :obj="obj"></div_purchase_center>
						</div>
					</div>
				</div>
			</div>
...
<script>
	import div_purchase_center from "@/components/diy/div_purchase_center.vue";
	...
	export default {
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/purchase_center/set?purchase_center_id=' + obj["purchase_center_id"], {
					hits
				}, (res) => {
					if(res.result){
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.purchase_center_id;
				}
			},
			/**
			 * 初始化收藏状态
			...
			get_collect(obj) {
				...
				var query = {
					source_table: "purchase_center",
					source_field: "purchase_center_id",
					source_id: obj["purchase_center_id"],
					user_id
				};

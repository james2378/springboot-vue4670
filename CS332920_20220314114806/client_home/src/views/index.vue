		        ...
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/purchase_center/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_purchase_center = !show_list_purchase_center">
		                        <div class="switch_box">
		                            <span v-if="show_list_purchase_center"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!--求购中心推荐列表组件 -->
		                <list_purchase_center v-if="show_list_purchase_center" :list="list_purchase_center" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_purchase_center.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_purchase_center" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_purchase_center" :key="i" @click="to_details('purchase_center',o,'purchase_center_id')">
		                            <td v-for="(oj,n) in fields_purchase_center">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
	...
	export default {
		...
		data() {
			return {
				...
				show_list_purchase_center: true,
				list_purchase_center: [],
				vm_arr_purchase_center: [
					"purchase_center_id",
					"buy_goods",
					"item_type",
					"item_picture",
		...
		methods: {
			...
			// 获取求购中心列表
			get_purchase_center() {
				let url = "~/api/purchase_center/get_list?";
				this.$get(url, {
					"page": 1,
					"size": 4
				}, (json) => {
					if (json.result) {
						this.list_purchase_center = json.result.list;
					}
				})
			},
		...
		computed: {
		    list_table_purchase_center() {
		        var list = this.list_purchase_center;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_purchase_center.map((o) => {
		                // 第二个中括号是对象的属性

<template>
	<div class="diy_details_box diy_div_purchase_center">
		<div class="warp">
			<div class="container">
					...
					<div class="view praise_wrap">
						...
						{{ praiseLen }}
							...
							<div class="details_btn_wrap">
								<button class="btn btn-primary details_btn" v-if="$check_action('/message_notification/edit','add')" @click="to_form('/message_notification/edit')" >留言 </button>
								<button class="btn btn-primary details_btn" v-if="$check_action('/purchase_center/details','set')" @click="change_praise()" >点赞</button>
								<button class="btn btn-primary details_btn" v-if="$check_action('/purchase_center/details','set')" @click="change_collect()" >收藏</button>
							</div>
						<!-- 富文本 -->
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "purchase_center_id",
					source_id: this.obj["purchase_center_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/purchase_center/set?purchase_center_id=' + _this.obj["purchase_center_id"], {
								praise_len
							}, (res) => {
								if(res.result){
			...
			change_collect() {
				...
				if (this.state_collect) {
					...
					query.title = this.obj.buy_goods

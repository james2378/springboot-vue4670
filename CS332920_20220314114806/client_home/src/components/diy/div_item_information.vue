<template>
	<div class="diy_details_box diy_div_item_information">
		<div class="warp">
			<div class="container">
	...
	export default {
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "item_information",
					source_field: "item_information_id",
					source_id: this.obj["item_information_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/item_information/set?item_information_id=' + _this.obj["item_information_id"], {
								praise_len
							}, (res) => {
								if(res.result){

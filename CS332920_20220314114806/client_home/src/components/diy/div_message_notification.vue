		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "message_notification_id",
					source_id: this.obj["message_notification_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/message_notification/set?message_notification_id=' + _this.obj["message_notification_id"], {
								praise_len
							}, (res) => {
								if(res.result){
